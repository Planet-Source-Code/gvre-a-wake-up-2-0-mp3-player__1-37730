...
notEx:
    If gLang = "Greek" Then
        MsgBox "Δέν βρέθηκε το απαιτούμενο αρχείο για το φόρτωμα της λίστας."
    Else
                ...
                If Form1.listviewOnAir.ListItems.count > 0 Then
                    For count% = 1 To Form1.listviewOnAir.ListItems.count
                        If Trim$(Mid$(source_form.File1.List(counter%), 1, Len(source_form.File1.List(counter%)) - 4)) = Trim$(Form1.listviewOnAir.ListItems(count%).SubItems(2)) Then
                            If gLang = "Greek" Then
                                ans% = MsgBox("Το τραγούδι ' " & Mid$(Form2.File1.List(counter%), 1, Len(Form2.File1.FileName) - 4) & " ' υπάρχει στην λίστα.Να προστεθεί ξανά;", vbYesNo + vbDefaultButton2)
                            Else
