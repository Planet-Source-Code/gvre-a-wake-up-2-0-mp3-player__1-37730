...
errhandler:
    Select Case Err
        Case 68
            res% = MsgBox("Η συσκευή δεν είναι διαθέσιμη.", vbAbortRetryIgnore)
            Select Case res%
                Case vbAbort
                    Exit Sub
                Case vbRetry
    ...
    If Form1.listviewOnAir.ListItems.count > 0 Then
        ...
        For count% = 1 To Form1.listviewOnAir.ListItems.count
                If Trim$(Mid$(File1.List(File1.Tag), 1, Len(File1.List(File1.Tag)) - 4)) = Trim$(Form1.listviewOnAir.ListItems(count%).SubItems(2)) Then
                    ans% = MsgBox("Το τραγούδι '" & Mid$(File1.List(File1.ListIndex), 1, Len(File1.FileName) - 4) & "' υπάρχει στην λίστα.Να προστεθεί ξανά;", vbOKCancel)
                    If ans% = vbOK Then
